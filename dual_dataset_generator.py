...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "verbose_listops", os.path.join(current_dir, "verbose-listops.py")
    )
    ...
    DEFAULT_COST_PER_MILLION_PROMPT_TOKENS = (
        verbose_listops.DEFAULT_COST_PER_MILLION_PROMPT_TOKENS
    )
    DEFAULT_COST_PER_MILLION_COMPLETION_TOKENS = (
        verbose_listops.DEFAULT_COST_PER_MILLION_COMPLETION_TOKENS
    )
...
def generate_validation_dataset(num_samples=100, max_workers=10):
            ...
            if sample:
                ...
                enhanced_sample = {
                    ...
                    "ground_truth": sample.get(
                        "ground_truth_answer", sample.get("ground_truth", 0)
                    ),
                ...
                # Show progress
                print(
                    f"\rGenerated validation sample {len(validation_samples)}/{num_samples}",
                    end="",
                )
        except Exception as e:
            script_logger.error(
                f"Error generating validation sample {i}: {e}", exc_info=True
            )
    ...
    if validation_samples:
        ...
        script_logger.info(
            f"Validation dataset generation completed in {time_taken:.2f} seconds. {len(validation_samples)}/{num_samples} samples saved."
        )
...
def generate_benchmark_dataset(num_samples=1000, max_workers=DEFAULT_MAX_WORKERS):
    ...
    script_logger.info(
        f"Generating {num_samples} benchmark samples using {max_workers} workers..."
    )
            ...
            try:
                ...
                if sample:
                    ...
                    minimal_sample = {
                        ...
                        "narrative_with_question": sample.get(
                ...
                print(
                    f"\rGenerated benchmark sample {completed}/{num_samples} ({progress_pct:.1f}%)",
                    end="",
                )
            except Exception as e:
    ...
    if benchmark_samples:
        ...
        time_taken = time.time() - start_time
        script_logger.info(
            f"Benchmark dataset generation completed in {time_taken:.2f} seconds. {len(benchmark_samples)}/{num_samples} samples saved."
        )
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(
                "Usage: python dual_dataset_generator.py [validation_samples] [benchmark_samples] [max_workers]"
            )
            return
    ...
    validation_output = generate_validation_dataset(
        validation_samples, max_workers=min(10, max_workers)
    )

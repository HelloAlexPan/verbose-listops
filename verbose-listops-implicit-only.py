...
4. Only use allowed numbers as specified in the rules"""
        ...
        validator_user_prompt = f"""Evaluate the 'Generated Beat Text' below with ABSOLUTE PRECISION.
**Context:**
...
Conceptual Inputs: {conceptual_inputs_str_for_llm_validator}
New Atomic Inputs: {atomic_inputs_words_str_for_llm_validator}
Expected Beat Result: {expected_beat_result_words_for_llm_validator if expected_beat_result_words_for_llm_validator else 'N/A'}

**VALIDATION ALGORITHM - FOLLOW EXACTLY:**
1. Extract ALL numbers from the text (words and digits)
2. REQUIRED NUMBERS (MUST ALL BE PRESENT):
   - Atomic inputs: {atomic_inputs_words_str_for_llm_validator}
   - {"Result must be IMPLICIT only (not stated)"}

3. ADDITIONALLY ALLOWED NUMBERS (MAY BE PRESENT):
   - Small phrasing numbers: {phrasing_numbers_val_str} 
   - Input count: '{num_to_words(current_op_arity)}' ({current_op_arity}) IS ALLOWED to count inputs

4. FORBIDDEN NUMBERS (MUST NOT APPEAR):
   - Any prior results: {forbidden_val_str}
   - Any number not in lists above
   - For MED: The median result ({expected_beat_result_words_for_llm_validator}) must NOT appear anywhere

5. RESULT HANDLING:
   - For intermediate beats with IMPLICIT results: The number '{expected_beat_result_words_for_llm_validator}' must NOT appear explicitly
   - For root node: The final answer must NOT be explicitly stated

**VALIDATION RESPONSE:**
Is the text VALID (passes ALL checks above) or INVALID (fails ANY check)?
1. If ANY number appears that isn't explicitly allowed by rules 2-3, the text is INVALID
2. If ANY required number from rule 2 is missing, the text is INVALID  
3. If a result that should be implicit is explicitly stated, the text is INVALID
4. If a result that should be explicit is not stated, the text is INVALID

Output a valid JSON response only. Start with {{ and end with }}.
...
def _generate_narrative_recursive(
    ...
    # AVG/SM exception
    avg_sm_exception_str = ""  # No intermediate sums allowed
    if False and node.op in ["AVG", "SM"] and direct_atom_sum is not None:
    ...
    if True:  # Always use implicit results
    ...
    if is_root:
        ...
        action_description_parts.append(
            ...
            f"For this operation, you MAY explicitly mention prior conceptual inputs by their numerical values. "
            f"You MAY ONLY show the direct sum of atomic inputs as an explicit number - no other intermediate calculations or combinations. "
            f"The final result after taking modulo 10 is {correct_result} ('{num_to_words(correct_result)}')."
        )
        if True:  # Always implicit results
            action_description_parts.append(
    ...
    # All results are implicit now (removed explicit result code)
    ...
    py_validator_enforce_result_presence = False  # Force implicit results ALWAYS  # Root result always implicit
    ...
    validate_beat_numbers = make_number_validator(
        ...
        conceptual_input_values=set(child_op_node_results_as_conceptual_inputs.values()) if child_op_node_results_as_conceptual_inputs else None,  # Pass conceptual input values

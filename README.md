...
Existing benchmarks inadequately test this narrative-embedded reasoning, often lacking realistic distractors or separate controls for context length and task complexity, thus masking a key LLM limitation. To address this, we introduce Verbose ListOps, a novel benchmark that agentically generates lengthy narratives around ListOps computations using post-order AST traversal and strict, step-wise numerical validation. It uniquely provides independent controls for narrative length and reasoning complexity with deterministic evaluation. 

Our experiments reveal a striking finding: state-of-the-art LLMs (GPT-4.1, Claude 3.5 Sonnet, Gemini 1.5 Pro) consistently fail Verbose ListOps at modest (~10k token) lengths, while easily solving the original ListOps task. While LongReason stands as the leading benchmark for general long-context multi-hop QA, our results show Verbose ListOps isolates a distinct failure mode: the inability to reason when logic is deeply embedded within coherent narrative containing relevant-yet-distracting elements. Verbose ListOps thus provides a necessary complement, offering a crucial tool to measure this specific narrative reasoning resilience, verifying core capabilities like operand identification, sub-task tracking, and distraction resistance essential for reliable downstream applications. Code and data will be released publicly.

...
LLMs excel at retrieving facts from long contexts but critically fail complex reasoning within coherent narratives, limiting its real world applications; like the automation of extracting qualification signals from sales transcripts. Existing benchmarks inadequately test this narrative-embedded reasoning, lacking realistic distractors or independent controls for context length versus task complexity, masking a key LLM limitation. We introduce Verbose ListOps, a novel benchmark that agentically generates lengthy narratives around [ListOps](https://arxiv.org/abs/1804.06028) (2018) computations, making it the first long-context QA eval that with independent controls for both context window size and reasoning complexity with deterministic evaluation. Our experiments reveal a striking finding: state-of-the-art LLMs (GPT-4.1, Claude 3.5 Sonnet, Gemini 1.5 Pro) consistently fail Verbose ListOps at modest (~10k token) lengths, despite easily solving the original ListOps task. 


While [LongReason](https://arxiv.org/html/2501.15089v1_) (2025) is todays leading benchmark for long-context QA, Verbose ListOps isolates a critical failure mode: reasoning amidst coherent narrative distraction. Understanding why is crucial for evaluating skills analogous to real-world signal extraction, such as pinpointing context-specific facts, tracking sub-task results, and resisting narrative distraction. Verbose Listops thus enables targeted improvements beyond simply increasing context window size, assessing progress on the specific narrative reasoning skills required for AGI to reliably automate complex, time-intensive human tasks beyond simple retrieval tasks. 

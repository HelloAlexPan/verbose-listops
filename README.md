...
Current Large Language Models (LLMs) excel at retrieving facts from long contexts, but critically fail complex reasoning tasks when embedded within coherent narratives. This gap limits their effectiveness in applications requiring deep understanding of unstructured text, like extracting qualification signals from sales transcripts.

Existing benchmarks inadequately test this narrative-embedded reasoning, often lacking realistic distractors or separate controls for context length and task complexity, thus masking a key LLM limitation. To address this, we introduce Verbose ListOps, a novel benchmark that agentically generates lengthy narratives around ListOps computations. This makes it the first synthetically generated Multi-Hop QA benchmark that has independent controls for both narrative length and reasoning complexity with deterministic evaluation.

Our experiments reveal a striking finding: state-of-the-art LLMs (GPT-4.1, Claude 3.5 Sonnet, Gemini 1.5 Pro) consistently fail Verbose ListOps at modest (~10k token) lengths, while easily solving the original ListOps task. While LongReason (2025) is the leading benchmark for general long-context multi-hop QA today, our results show Verbose ListOps isolates a distinct failure mode: the inability to reason when logic is deeply embedded within coherent narrative containing relevant-yet-distracting elements. Verbose ListOps is therefore crucial for testing a model's ability to perform tasks analogous to real-world signal extraction. It measures core skills like identifying specific numerical inputs required only for the immediate calculation step described within the narrative, much like pinpointing a specific qualification signal (e.g., budget confirmation) within a long sales transcript, while correctly tracking sub-task results and resisting narrative distraction. Crucially, Verbose ListOps provides the first reliable metric to diagnose why models fail complex narrative reasoning, enabling targeted improvements beyond simply increasing context window size, guiding development towards AGI that can utilize its reasoning to replace highly time-intensive human tasks. Code and data will be released publicly.

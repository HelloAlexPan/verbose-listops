...
def main():
    ...
    if not os.path.exists(dual_dataset_generator_path):
        print(
            f"Error: dual_dataset_generator.py not found at {dual_dataset_generator_path}"
        )
        return 1
    ...
    validation_samples = min(
        100, max(20, int(num_samples) // 10)
    )  # 10% of main, between 20-100
    benchmark_samples = min(
        1000, max(100, int(num_samples))
    )  # Similar to main size, capped at 1000
